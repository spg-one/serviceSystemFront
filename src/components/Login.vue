<template>
  <div>
    <div
      style="width:100%;height:30px;display:flex;flex-direction:row;justify-content:center;border-bottom:1px solid #ddd;margin-bottom:45px;"
    >
      <div>
        <span
          style="color:black;background:white;font-size:38px;line-height:58px;padding:0 20px;"
          >登录</span
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    login() {
      ...
      this.loading = true;
      this.axios
        .post("/api/login/login-post", param)
        .then(res => {
          that.disabled = true;
          that.loading = false;
          Message.success({
